...
GNEWS_KEYS = ["115a332e2a8ff64097095dacb118fb0b","9eae6c4d2ca2696fd7ba29501743691d", "e5ae761e07751030d20889841668d7af"
...
async def try_gnews_api(ticker: str, limit: int = 10):
    key = random.choice(GNEWS_KEYS)  # Rotates keys like other sources
    url = (
        f"https://gnews.io/api/v4/search"
        f"?q={ticker}"
        f"&lang=en"
        f"&country=us"
        f"&max={limit * 2}"  # Overfetch to ensure enough after filtering
        f"&apikey={key}"
    )
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=headers) as resp:
                if resp.status != 200:
                    print(f"[gnews error] Status: {resp.status}")
                    return []
                data = await resp.json()
    except Exception as e:
    ...
    for article in data.get("articles", []):
        ...
        # Keep the exact same date filtering as your old working snippet
        try:
            pub_date = datetime.fromisoformat(article["publishedAt"].replace("Z", "+00:00")).date()
        except:
            continue
        if pub_date not in [datetime.utcnow().date(), (datetime.utcnow() - timedelta(days=1)).date()]:
            continue

        # Try full text extraction first, then fall back to description
        text, _ = await extract_article_text_fallback_async(article["url"])
        if not text:
...
async def get_articles(ticker, limit=2, max_attempts=2):
    ...
    for attempt in range(max_attempts):
        ...
        random.shuffle(sources)  # randomize source order each attempt
        ...
        for source_func in sources:
            ...
            if articles:
                # Return first working source as DataFrame
                return pd.DataFrame(articles)[["date", "title", "text", "url", "ticker"]]

    print(f"❌ All sources failed for {ticker} after {max_attempts} attempts.")
