...
# # ------------------ SQLite Cache ------------------ #
# DB_PATH = "db/sentiment_cache.db"
# os.makedirs("db", exist_ok=True)

# def get_cached_sentiment(ticker: str, max_age_minutes=30):
#     try:
#         conn = sqlite3.connect(DB_PATH)
#         cursor = conn.cursor()
#         cursor.execute("SELECT sentiment, timestamp, article_blurb, article_urls FROM sentiment_cache WHERE ticker = ?", (ticker,))
#         row = cursor.fetchone()
#         conn.close()
#         if not row:
#             return None
#         sentiment, timestamp_str, blurb, urls_json = row
...
#                 "timestamp": timestamp,
#                 "article_blurb": blurb,
#                 "article_urls": json.loads(urls_json)
#             }
#     except Exception as e:
#         print(f"[DB ERROR] Failed to read cache: {e}")
#     return None

# def store_sentiment_cache(ticker, sentiment, blurb, urls):
#     try:
#         conn = sqlite3.connect(DB_PATH)
#         cursor = conn.cursor()
#         cursor.execute("""
...
#         """)
#         cursor.execute("""
#             INSERT OR REPLACE INTO sentiment_cache (ticker, sentiment, timestamp, article_blurb, article_urls)
#             VALUES (?, ?, ?, ?, ?)
#         """, (
#             ticker,
#             float(sentiment),
...
#         conn.close()
#     except Exception as e:
#         print(f"[DB ERROR] Failed to store cache: {e}")
...
#         cached = get_cached_sentiment(tk)
#         if cached:
#             results.append({
#                 "ticker": tk,
...
#         store_sentiment_cache(tk, sentiment_score, blurb, article_urls)
